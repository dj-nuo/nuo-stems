...
**4.1.0-alpha.2** - Apr 9, 2026
- fix: mp3 files failing to convert to Serato stems

**4.1.0-alpha.1** - Apr 8, 2026 🎉 **Serato integration** 🎉
- feat: Serato integration (ability to create `.serato-stems` files)

**4.0.0** - Mar 30, 2026
- fix: prevent TP4 linked stems affecting Beatport streaming duplicates with same AUDIO_ID.

**4.0.0-beta.64** - Mar 24, 2026
- fix: tracks failing to process on AMD & Intel GPUs on Windows due to insufficient GPU memory; Now NS4 falls back to CPU processing for these tracks/GPUs.
- fix: improve error message when Traktor Stems Folder doesn't exist on disk
- fix: MUTAGEN_PROCESS_FAILED error sometimes caused by some tricky tags in original tracks
- refactor: optimized data transfer for Traktor collection & queue. Performance might be slightly improved (I haven't noticed a change myself)

**4.0.0-beta.63** - Mar 15, 2026
- feat: "save .wav" now has FLAC option
- feat: add badge & tooltip for missing AUDIO_ID tracks on TP4 page
- fix: some weird directory names causing AUDIO_CONVERTER_PROCESS_FAILED
- fix: when output folder was set in non-TP4 mode, and then folder got deleted - when doing conversions in TP4 mode it triggered an error for inexistence of output folder even if "advanced" mode was disabled
- misc: on TP4 page all folders are now collapsed by default
- fix: show Blog popup only once after "Understand" clicked
- fix: "File Path" SMART Playlist working incorrectly
- fix: migrating stem.m4a to linked stems stopped every 3 hours
- fix: "delete" lonely stems when doing "Repair Collection"

**4.0.0-beta.62** - Mar 13, 2026 - 🤯 **AMD & Intel GPU support and performance improvements** 🤯
- feat: DirectX 12 GPU acceleration support for AMD and Intel GPUs on Windows. Requires FL 12.0 or higher
- feat: NVIDIA GPUs and Apple Silicon also benefit from further optimization, with typical speed gains of around 10–20%.
- feat: On some older NVIDIA cards, such as the GTX 1650, the improvement is even more dramatic, reaching roughly 2x to 2.5x faster performance.

**4.0.0-beta.61** - Feb 13, 2026
- fix: some NVIDIA CUDA GPUs with 4GB VRAM not being detected on Windows
- fix: **super long tracks** (e.g. 30 min+) causing app to crash. Memory is now much more controlled and doesn't depend on the track length.

**4.0.0-beta.60** - Feb 9, 2026
- fix: Multiple "fastlist.exe" processes spawned on Windows causing system to hang 

**4.0.0-beta.59** - Feb 3, 2026
- feat: when installing NS4, install Microsoft Visual C++ Redistributable if not already installed on Windows
- feat: COMMENT2 tag now also set when doing "migrate .stem.m4a to linked stems"
- fix: renderer memory leaks fixed by reloading window every 3 hours
- feat: dialog window if there are existing .stem.m4a in the output folder when in Legacy mode
- feat: cache macOS root volume name to improve performance in getMacOSRootVolumeName function
- fix: repair collection doesn't repair tracks if there are duplicate AUDIO_IDs in the collection.

**4.0.0-beta.58** - Jan 25, 2026
- feat: Traktor Pro 4 **SMART Playlists** support on TP4 page
- feat: new setting "Include Instrumental Stem" when saving WAV files
- feat: new UX for Traktor collection repair functionality and settings management.
- feat: Index (#) column option on TP4 page

**4.0.0-beta.57** - Jan 23, 2026
- fix: another potential fix for memory leak crashes when NUO-STEMS is running for a long time (12+ hours)

**4.0.0-beta.56** - Jan 18, 2026 - ✅ **New Website** ✅
- fix: potential fix for memory leak crashes when NUO-STEMS is running for a long time (12+ hours)
- fix: improve error handling and directory creation when saving final .stem.m4a/.mp4
- feat: configure app DNS to use Cloudflare for enhanced security
- feat: license key activation form for new users
- style: Stop Processing button
- feat: implement first launch detection and onboarding link opening
- style: BlogNewsDialog removed border & changed bg
- misc: don't show latest blog news on first app launch after install
- misc: demo mode TP4-native error. Added "Please switch to Legacy or DAW mode."
- fix: Clearing queue doesn't stop processing
