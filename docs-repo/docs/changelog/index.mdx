...
- fix: memory leaks fixed by reloading window every 3 hours (it doesn't stop the queue processing)
- feat: dialog window if there are existing .stem.m4a in the output folder when in Legacy mode
- misc: slightly improved speed of adding tracks to the queue on macOS.
